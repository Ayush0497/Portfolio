...
public class EditorMenuPathnodes : MonoBehaviour
{
    ...
    static void BuildPaths()
    {
        ...
        for (int i = 0; i < allSpheres.Length; i++)
        {
            ...
            {
                if (Vector3.Distance(allSpheres[i].transform.position, allSpheres[j].transform.position) <= pathDistance && i != j && !Physics.SphereCast(allSpheres[i].transform.position + new Vector3(0, 0.5f, 0), 0.4f, allSpheres[j].transform.position - allSpheres[i].transform.position, out hit, 2.0f))
                {
